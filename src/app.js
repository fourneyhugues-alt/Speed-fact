...
function safeFileName(v){return String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9_-]+/g,'_').replace(/^_+|_+$/g,'').slice(0,60)||'client'}
function loadHtml2Pdf(){
  if(window.html2pdf)return Promise.resolve(window.html2pdf);
  if(html2pdfLoader)return html2pdfLoader;
  html2pdfLoader=new Promise((resolve,reject)=>{
    const existing=[...document.scripts].find(x=>String(x.src||'').includes('html2pdf.bundle.min.js'));
    if(existing){existing.addEventListener('load',()=>window.html2pdf?resolve(window.html2pdf):reject(new Error('html2pdf indisponible')),{once:true});existing.addEventListener('error',()=>reject(new Error('Chargement PDF impossible')),{once:true});return}
    const s=document.createElement('script');
    s.src='https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js';
    s.async=true;s.crossOrigin='anonymous';s.referrerPolicy='no-referrer';
    s.onload=()=>window.html2pdf?resolve(window.html2pdf):reject(new Error('html2pdf indisponible'));
    s.onerror=()=>reject(new Error('Impossible de charger le générateur PDF. Vérifie la connexion internet.'));
    document.head.appendChild(s);
  });
  return html2pdfLoader;
}
function waitForImages(root){return Promise.all([...root.querySelectorAll('img')].map(img=>img.complete?Promise.resolve():new Promise(r=>{img.addEventListener('load',r,{once:true});img.addEventListener('error',r,{once:true})})))}
function downloadSharedFile(file){const u=URL.createObjectURL(file),a=document.createElement('a');a.href=u;a.download=file.name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),1500)}
async function buildInvoicePdfFile(){
  if(!currentInvoice)throw new Error('Aucune facture ouverte.');
  await loadHtml2Pdf();
  const visible=$('invoiceDocument');
  if(!visible)throw new Error('Document facture introuvable.');
  const holder=document.createElement('div');
  holder.className='speedfact-pdf-export';
  holder.style.cssText='position:fixed;left:-12000px;top:0;width:794px;background:#fff;z-index:-1000;pointer-events:none;';
  const doc=visible.cloneNode(true);doc.style.cssText='width:794px;max-width:794px;margin:0;padding:0;background:#fff;';holder.appendChild(doc);
  const st=document.createElement('style');
  st.textContent=`.speedfact-pdf-export .invoice-document{width:794px!important;max-width:794px!important;padding:0!important;margin:0!important;background:#fff!important}.speedfact-pdf-export .invoice-page,.speedfact-pdf-export .annex-page{width:794px!important;min-height:1122px!important;padding:30px!important;box-sizing:border-box!important;background:#fff!important}.speedfact-pdf-export .invoice-parties{grid-template-columns:1fr 1fr!important}.speedfact-pdf-export .invoice-bottom{grid-template-columns:1.3fr .8fr!important}.speedfact-pdf-export .invoice-table{display:table!important;overflow:visible!important;white-space:normal!important}.speedfact-pdf-export .invoice-table th,.speedfact-pdf-export .invoice-table td{white-space:normal!important}.speedfact-pdf-export .invoice-watermark{width:68%!important}.speedfact-pdf-export .annex-grid,.speedfact-pdf-export .audit-grid{grid-template-columns:1fr 1fr!important}.speedfact-pdf-export .annex-page{page-break-before:always!important;break-before:page!important}.speedfact-pdf-export .invoice-page{page-break-after:always!important;break-after:page!important}`;
  document.head.appendChild(st);document.body.appendChild(holder);
  try{
    await waitForImages(holder);
    await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
    const fileName=`Facture_${safeFileName(currentInvoice.num)}_${safeFileName(currentInvoice.client)}.pdf`;
    const blob=await window.html2pdf().set({margin:0,filename:fileName,image:{type:'jpeg',quality:.96},html2canvas:{scale:1.35,useCORS:true,backgroundColor:'#ffffff',logging:false,windowWidth:794,scrollX:0,scrollY:0},jsPDF:{unit:'mm',format:'a4',orientation:'portrait',compress:true},pagebreak:{mode:['css','legacy'],before:'.annex-page'}}).from(doc).outputPdf('blob');
    return new File([blob],fileName,{type:'application/pdf',lastModified:Date.now()});
  }finally{holder.remove();st.remove()}
}
async function sendInvoice(){
  if(!currentInvoice)return;
  const btn=[...document.querySelectorAll('.invoice-toolbar button')].find(b=>b.textContent.includes('ENVOYER'));
  const oldText=btn?.textContent;if(btn){btn.disabled=true;btn.textContent='CRÉATION DU PDF…'}
  try{
    const file=await buildInvoicePdfFile();
    const issuer=currentInvoice.issuerSnapshot?.name||'Fourney Hugues';
    const shareData={title:`Facture ${currentInvoice.num} - ${currentInvoice.client}`,text:`Bonjour,\n\nVeuillez trouver la facture ${currentInvoice.num} accompagnée de toutes les fiches chantier correspondantes.\n\nCordialement,\n${issuer}`,files:[file]};
    const canShareFiles=!!(navigator.share&&navigator.canShare&&navigator.canShare({files:[file]}));
    if(canShareFiles){
      if(btn)btn.textContent='CHOISIS GMAIL…';
      await navigator.share(shareData);
      const sent=confirm('Le mail avec le PDF a-t-il bien été envoyé depuis Gmail ?');
      if(sent){finalizeInvoice();alert('Facture marquée comme envoyée. Le PDF contenait la facture + toutes les fiches chantier.')}else{alert('La facture reste non envoyée dans Speed Fact. Tu pourras recommencer plus tard.')}
    }else{
      downloadSharedFile(file);
      alert('Le partage direct de fichier n’est pas disponible ici. Le PDF complet vient d’être téléchargé : facture + toutes les fiches chantier. Tu peux le joindre dans Gmail.');
    }
  }catch(e){
    if(e?.name==='AbortError')return;
    console.error(e);alert(`Impossible de préparer le PDF : ${e?.message||'erreur inconnue'}`);
  }finally{if(btn){btn.disabled=false;btn.textContent=oldText||'ENVOYER LE PDF'}}
}
...
function enhanceStaticUi(){
  ...
  const hero=document.querySelector('.hero');if(hero){hero.classList.add('hero-photo');hero.style.backgroundImage="linear-gradient(180deg,rgba(5,12,24,.12),rgba(5,12,24,.70)),url('assets/home-hero.jpg')";hero.style.backgroundSize='cover';hero.style.backgroundPosition='center center';const hs=document.createElement('style');hs.textContent='.hero.hero-photo:before,.hero.hero-photo:after{display:none!important}';document.head.appendChild(hs)};
